...
public class Take_Order_Activity_Offline extends Activity implements View.OnClickListener, SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        gps = new GPSTracker(Take_Order_Activity_Offline.this);
        ...
        databaseHelper = new DatabaseHelper(Take_Order_Activity_Offline.this);
        try {
            findLaction();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    CostChangedListener costChangedListener = new CostChangedListener() {
        @Override
        public void onCostChanged(double change, double qty) {

            totleprice = round(change, 2);
            change = round(change, 2);
            if (change == -0.00) {
                totleprice = 0.0;
                change = 0.0;
                //String.format("%.2f", d)
                tv_Tprice.setText("PKR. " + String.format("%.2f", change));
                totleprice = Double.parseDouble(String.format("%.2f", change));
            } else {
                totleprice = Double.parseDouble(String.format("%.2f", change));
                tv_Tprice.setText("PKR. " + String.format("%.2f", change));
            }
    ...
    void fun_SetData() {
        adapter_tAkeOrder = new ItemListAdapter(Take_Order_Activity_Offline.this, 0, MyConstants.mArrayProducts, costChangedListener);
        ...
        switch (v.getId()) {
            ...
            case R.id.save_order:
                ...
                if (MyConstants.mArrayProducts_selected.size() > 0) {
                    Intent intent = new Intent(getApplicationContext(), View_Order_Activity_Offline.class);
    ...
    void Gernal_Dialog(String Title1, String Title, String strBtn, String strBtn2) {
        ...
        // custom dialog
        final Dialog dialog = new Dialog(Take_Order_Activity_Offline.this);
    ...
    void setDialoguData() {
        ...
        cityListDialog = new CityListDialog(Take_Order_Activity_Offline.this, array, tv_customer, 200);
    }
